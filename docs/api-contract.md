...
`provider` is a small string enum returned by `/api/feedback`, `/api/summary`, and the chat endpoint's `quick_feedback`. It is purely informational — `"llm"` means the response came from a successful real-provider call, `"mock"` means the deterministic rule-based fallback produced it (because `LLM_PROVIDER_MODE` is not `llm`, required env vars are missing, or the live call failed). The backend only ever logs the provider mode and a short fallback-reason summary (e.g. exception type), never the API key, request payload, or raw error text.
